...
The same audit ID subsequently recovered in place as the **post-Article-324 recrawl**, with `crawl_state=completed`, `postprocessing_status=completed`, `is_terminal=true`, `should_repoll=false`, a new start of `2026-09-23T19:11:54.784688Z` and completion of `2026-09-23T20:53:07.039255Z`. Its score is 970, with 449 successful pages, 40 redirects, 448 indexable, 41 non-indexable and 2,429 raw observations. This terminal tuple and later timestamps supersede the earlier nonterminal snapshot, whose populated aggregate completion field still referred to the prior crawl. Recovery initiator/root cause remain unknown; Codex did not trigger a second crawl.

The fresh post-recrawl OTTO dashboard is **906 total / 16 approved-deployed / 890 pending**, SEO score 81. Its pending panels comprise 492 on-page, 20 Dynamic Indexing and 378 social-metadata rows; the 16 approved-deployed rows are Topical Maps and Supplemental Content. These dashboard rows are not the 2,429 raw audit observations, and none of the 890 pending rows was auto-deployed. Prior generated-proposal reviews remain separate.

Article 324 has **no exact completed-crawl Page Explorer row** yet. Articles 322 and 323 also lack independently observed exact successful/indexable rows. Therefore `config/mrx1000-incremental-production-verification.json` and the final-recrawl-verified count remain **321**. The hash-locked historical release-10 canonical JSON/CSV remain unchanged. Obtain each exact page-row proof before advancing the ledger; the aggregate success/indexability counts are insufficient to identify those URLs. The extension-backed Chrome Page Explorer surface was unavailable at the time of this update, so no per-URL crawl claim is made.
